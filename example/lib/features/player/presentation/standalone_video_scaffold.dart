...
part 'standalone_video_option_panels.dart';
...
part 'standalone_video_scaffold_sheets.dart';

enum StandaloneLoopMode { none, single, playlist }

class StandaloneVideoScaffold extends StatefulWidget {
  const StandaloneVideoScaffold({
    super.key,
  ...
  final StandaloneLoopMode loopMode;
  ...
  final ValueChanged<StandaloneLoopMode> onLoopModeChanged;
  @override
  State<StandaloneVideoScaffold> createState() =>
      _StandaloneVideoScaffoldState();
}

class _StandaloneVideoScaffoldState extends State<StandaloneVideoScaffold> {
  ...
  @override
  void didUpdateWidget(covariant StandaloneVideoScaffold oldWidget) {
  ...
  Widget build(BuildContext context) {
    ...
    final gestureConfig = widget.controlsLocked || _sidePanel != null
        ...
        : const M3u8GestureControlsConfig(
            // The gesture package's persistent brightness dimmer is intended
            // for apps that render their own brightness chrome. Keeping it on
            // makes the hidden-control state look permanently veiled because
            // its initial brightness is 0.5. Gesture adjustment and its
            // transient value indicator remain enabled.
            brightnessOverlayEnabled: false,
          );
